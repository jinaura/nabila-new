(function (factory) {
	...
	$("#sortBy .sort__option").on("click", function () {
	  Shopify.queryParams = {};
	...
	$(".product-single__variants").on("change", function () {
	  const $this = $(this);
	  const thisPrice = $this.find("option:selected").data("price");
	...
	$.fn.animatedClass = function (options) {
	  ...
	  if (this.length > 0) {
	    return this.each(function () {
	      var winScroll = $win$2.scrollTop();
	      var ele = $(this);

	      if (ele.data('delay')) {
	        options.deviation = $win$2.height() - ele.data('delay');
	      }
	...
	$('.animate').animatedClass({
	  deviation: $win$2.height() - $win$2.height() * 0.2
	});
	$win$2.on('scroll', function () {
	  $('.animate').animatedClass({
	    deviation: $win$2.height() - $win$2.height() * 0.2
	  });
	});
	...
	$('.util-search a').click(function (e) {
	  e.preventDefault();

	  if ($('.util-search__form').hasClass('open')) {
	    $('.util-search__form form').submit();
	  } else {
	    $('.util-search__form').addClass('open');
	  }
	});
	$('.util-search__form .close').click(function (e) {
	  e.preventDefault();
	  $('.util-search__form').removeClass('open');
	});
	...
	const $fixedBar = $('.fixed-form.hidden-xs.product-fixed-bar');
	...
	function fixedBar() {
	  ...
	  const productBottomLine = $productForm.offset().top + $productForm.height();
		if (winScrollTop > productBottomLine) {
			$fixedBar.removeClass('hidden');
		} else {
			$fixedBar.addClass('hidden');
		}
	}
